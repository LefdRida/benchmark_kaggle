...
def kernel_local_CKA(base_source, base_target, query_source, query_target, device):
    cka = KernelCKA(base_target.shape[0]+1, device)
    
    graph = []
    
    for source_ind in tqdm(range(query_source.shape[0])):
        source = query_source[source_ind:source_ind+1, :]
        row = []
        for target_ind in range(query_target.shape[0]):
            target = query_target[target_ind:target_ind+1, :]
    
            row.append(cka.calculate(torch.cat((source, base_source), axis = 0), torch.cat((target, base_target), axis = 0)))
        graph.append(row)
    return torch.tensor(graph)
