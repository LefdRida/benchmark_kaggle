...
class RetrievalTask(AbsTask):
    ...
    def run(self, method: AbsMethod, support_embeddings: Dict[str, np.ndarray] = None, **kwargs) -> Dict[str, Any]:
        ...
        if hasattr(method, 'retrieve'):
            ...
            for idx in tqdm(range(aligned_queries[:9535].shape[0])):
                gt_query_ids = self.gt_ids[idx]
                q_emb = aligned_queries[idx, :].reshape(1, -1)
                sim_scores = similarity_function(q_emb, aligned_documents[:47675])
                # Get topk indices
                sim_top_idx = torch.topk(sim_scores, self.topk, dim=-1, largest=True, sorted=True).indices.cpu().numpy().flatten()
